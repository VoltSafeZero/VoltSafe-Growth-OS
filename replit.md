...
- **Conversion test suite**: `tests/conversion.test.js` — 13 assertions: convert-check, permission enforcement, create new org + verify status/convertedFromLeadId, double-convert → 400, unconvert preserves org, link existing org, empty-company guard → 400, linked-org endpoint.
- **Organization ↔ Lead linked record visibility (Phase 2b)**: `GET /api/accounts/:id/source-lead` (crm=view) returns `{ lead, history }` — source lead full record + all migration_map entries for that lead ordered newest-first. `AccountDetailDialog` Details tab shows collapsible "Source Lead" section (expanded by default) with lead ID, company, contact name/email/phone, city/region, stage-at-promotion from migrationMap notes, created date, truncated notes, and a secondary collapsible "Conversion History" timeline with action labels, timestamps, and org names per migrationMap entry. Section only renders when `convertedFromLeadId` is set. All read-only.
- **Email → CRM record linking UX (Phase 3)**: CRM Context Panel (bottom of Gmail thread view) now provides full click-through navigation to linked CRM records. `CrmContextPanel` accepts `userPermissions` and `isAdminUser` props (passed from `GmailInboxPage`). Deep-link URLs: `account` → `/accounts?selected={id}`, `lead`/`opportunity` → `/opportunities?selected={id}`, `contact` → `/contacts?selected={id}` (highlight + scroll), `partner` → `/strategy/partnerships?selected={id}` (dialog open). Navigation uses wouter `setLocation`. `getDeepLinkUrl()` and `RESTRICTED_LABELS` helpers added. Permission gating: if user lacks access for a record type, the chip shows a generic label ("Linked Organization", "Linked Lead", etc.) + lock icon, non-clickable — actual record name is never exposed. `crm` access gates account/lead/contact; `partnerships` gates partner. Admins bypass all. Match reasons from `association_reason_json` now surface in the UI: first reason shown as muted italic text below each chip; full reason list in `title` tooltip. Unconfirmed candidates are also navigable (click to peek before confirming/rejecting). `ContactsPage` (`contacts.tsx`): added `?selected={id}` URL deep-link — highlights the matched row with a primary ring border, scrolls it into view. `PartnershipsPage` (`partnerships.tsx`): added `?selected={id}` URL deep-link — fetches partner by ID via `GET /api/partnerships/:id`, opens detail dialog on mount. No schema changes. No new endpoints.
- **Bidirectional Org → Lead navigation (Phase 2d)**: "Promoted from Lead #N" badge in `AccountDetailDialog` header is now a clickable button (when `onOpenLead` prop is provided). "View full lead record →" link added inside the Source Lead panel card. `onOpenLead?: (leadId: number) => void` prop added to `AccountDetailDialog`. Two implementations: (A) from `AccountsPage` — closes account dialog, calls `setLocation('/opportunities?selected={id}')` so LeadsPage mounts and auto-opens the lead via URL param; (B) from `LeadsPage` — closes org dialog, fetches lead by ID, sets `selectedLead` to open `LeadDetailDialog` inline. `LeadDetailDialog` stage selector is now `disabled` when `canEdit=false` for read-only contexts. No circular imports: `LeadDetailDialog` stays private in `leads.tsx`; only the callback is passed via props.
- **Bidirectional Lead ↔ Organization navigation (Phase 2c)**: "Promoted to Organization" banner in `LeadDetailDialog` is now clickable (pointer cursor, hover `bg-emerald-500/10`, `ExternalLink` icon affordance). Click closes the lead dialog, fetches the org via `GET /api/accounts/:id`, and opens `AccountDetailDialog` in read-only or edit mode depending on `canEdit`. If the org was deleted after promotion, the banner shows a non-clickable "Organization unavailable" muted state. `AccountDetailDialog` exported from `accounts.tsx` and imported into `leads.tsx`. State: `pendingOrgId` + `useQuery` + `useEffect` pattern in `LeadsPage`. Permission-aware: crm=view users see the org in read-only; users without any CRM access never see the banner. `DELETE /api/accounts/:id` route added (was missing — was silently falling through to Vite HTML). Tests: T10 (GET /api/accounts/:id returns full account), T11 (linked-org null when org deleted), T12 (crm=view can GET but not DELETE).
