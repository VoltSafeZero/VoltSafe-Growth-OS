...
- **Email Ownership System (Phase 1 multi-user ready):** All `email_messages` rows carry `owner_user_id` (Trevor=4 backfilled) and `source_account_id`. `email_accounts` table tracks one connected Gmail per user. Gmail sync stamps every new email with `ownerUserId: TREVOR_USER_ID`. Domain routing engine (`server/services/email-folder-router.ts`) runs after CRM association for each incoming email.
- **Custom Inbox Folders:** `mail_folders`, `mail_folder_domains`, `email_folder_assignments` tables. Folders are per-user, hold domain rules with `ends_with` subdomain matching (e.g. `nmma.org` catches `events.nmma.org` but NOT `fake-nmma.org`). Gmail inbox sidebar shows Custom Folders section with unread/total counts. Folder settings dialog supports domain CRUD and async backfill reprocessing. Manual remove-from-folder supported. UI: `tab="folder"` view in gmail-inbox.tsx.
