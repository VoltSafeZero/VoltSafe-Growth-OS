...
- **Outbound Email Engagement Tracking:** Privacy-safe open/click tracking injected into all CRM-outbound emails. Schema: `email_tracking_pixels` (one row per sent email — `tracking_id`, `gmail_message_id`, `subject`, `recipient_email`, `sent_by_user_id`), `email_engagement_events` (per event — type: open|click, `ip_hash` SHA-256+salt, `user_agent` up to 500 chars, `is_bot`, `timeline_created`), `email_engagement_rules` (table-driven automation — trigger_type, min_events, action_type: create_notification|create_task, action_config JSONB). Service: `server/tracking.ts` — `generateTrackingId()` (UUID), `hashIp()` (HMAC-SHA256 16-hex, salt=`TRACKING_SALT` env), `isBotUserAgent()` (20+ patterns incl. Googlebot, Yahoo Mail proxy, Outlook SafeLinks, headless Chrome), `injectTracking()` (rewrites `<a href="http...">` → tracked redirect, appends 1×1 GIF pixel). Public routes (no auth): `GET /track/open/:trackingId.gif` (responds immediately with GIF, then fire-and-forget recordOpen), `GET /track/click/:trackingId?url=...` (302 redirect with URL validation, then recordClick). Auth routes: `GET /api/email-engagement/:trackingId`, `GET /api/email-engagement/by-message/:gmailMessageId`. Rules CRUD: `GET|POST|PATCH|DELETE /api/email-engagement-rules`. Downstream effects (first real event only): creates `email_engagement` activity on all CRM records linked via `email_associations`; fires notification + optional task via automation rules. Frontend: `EmailsTab` shows "Tracked" badge on outbound email rows; expanded panel shows live engagement stats (opens, unique opens, clicks, first opened, event timeline) via `EngagementPanel` component. Send injection: `POST /api/gmail/send` injects tracking before send (non-fatal — falls back to untracked on error), saves pixel record after send. Test suite: `tests/email-engagement.test.js` (23 assertions).
