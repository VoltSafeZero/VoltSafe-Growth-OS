...
- **Sales & Marketing Assets CMS:** Full asset library at `/assets` route. `assets` table stores name, description, category (document/spreadsheet/presentation/image/other), fileName, fileSize, mimeType, uploadedAt. Files stored in `uploads/assets/`. CRUD API: `GET/POST /api/assets`, `GET/PATCH/DELETE /api/assets/:id`, `GET /api/assets/:id/file` (download), `GET /api/assets/:id/download` (force download). UI: upload dialog with drag-and-drop, category tabs, search, grid view with image thumbnails and file-type icons, preview/download/delete per card. Asset picker integrated into ComposeDialog in Gmail Inbox — select assets as email attachments. Gmail `buildMimeRaw` supports `multipart/mixed` with binary attachments (`MimeAttachment[]`). Sidebar link under "Sales" section.
