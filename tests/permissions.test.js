 ...
 *
 * ISOLATION STRATEGY (safe — no real user mutation):
 *   Creates a temporary fixture viewer user (email @example.invalid) at startup
 *   with the exact viewer-level permissions needed by the test.  The real
 *   viewer@voltsafe.com user is never touched.  The fixture user is deleted in
 *   the finally block whether the test passes or fails.
 *
 *   mixed@voltsafe.com and trevor@voltsafe.com are used read-only (login + GET
 *   / POST to verify permission grants) — their passwords and permissions are
...
/** Viewer-level permissions: crm=view, everything else=none */
...
async function run() {
  ...
  // ── Create fixture viewer user ────────────────────────────────────────────
  // A new user with viewer-level permissions.  Email ends in @example.invalid
  // so it is immediately identifiable as test data and cannot be confused with
  // a real VoltSafe user.  Deleted unconditionally in the finally block.
  const FIXTURE_VIEWER_EMAIL = fixtureEmail("permtest", "viewer");
  const FIXTURE_VIEWER_PWD   = "permtest-viewer-" + Date.now();
  let   fixtureViewerId      = null;

  try {
    const pwdHash   = await bcrypt.hash(FIXTURE_VIEWER_PWD, 10);
    const insertRes = await pool.query(
      `INSERT INTO users
         (name, email, password, role, status, must_change_password, permissions)
       VALUES ($1, $2, $3, 'read-only', 'active', false, $4::jsonb)
       RETURNING id`,
      [
        "PermTest Fixture Viewer",
        FIXTURE_VIEWER_EMAIL,
        pwdHash,
        JSON.stringify(VIEWER_PERMISSIONS),
      ],
    );
    fixtureViewerId = insertRes.rows[0].id;
    console.log(`Fixture viewer created: id=${fixtureViewerId} email=${FIXTURE_VIEWER_EMAIL}\n`);

    // ── VIEWER USER (crm=view, all others=none) ───────────────────────────
    console.log("── Viewer (fixture | crm=view, all others=none) ──");
    const viewerCookie = await login(FIXTURE_VIEWER_EMAIL, FIXTURE_VIEWER_PWD);
    ...
    // Projects blocked (projects=none)
    ...
    // Knowledge/assets blocked (knowledge=none)
    ...
    // Communications blocked (communications=none)
    ...
    // Team workload blocked (team_workload=none)
    await check("GET  /api/team-workload      [wkld=none → 403]", v("/api/team-workload"), 403);

    // ── MIXED USER (crm=edit, quoting=view, support=edit, knowledge=view) ──
    ...
    // ── PATCH Permissions Validation (admin operates on fixture viewer) ───
    // Uses the fixture viewer's ID — avoids mutating any real user's permissions.
    console.log("\n── PATCH Permissions Validation (admin, targets fixture viewer) ──");
    await check(
      "PATCH /api/admin/users/:id/permissions [bad payload → 400]",
      a(`/api/admin/users/${fixtureViewerId}/permissions`, { method: "PATCH", body: JSON.stringify({ crm: "superuser" }) }),
      400,
    );
    await checkOneOf(
      "PATCH /api/admin/users/:id/permissions [good payload → 200]",
      a(`/api/admin/users/${fixtureViewerId}/permissions`, {
        method: "PATCH",
        body:   JSON.stringify({
          crm: "view", quoting: "none", support: "none", calendar: "none",
          projects: "none", knowledge: "none", mail_team: {}, partnerships: "none",
          calendar_team: [], team_workload: "none", communications: "none",
        }),
      }),
      200,
    );
    ...
    // ── ADMIN PRIVILEGE ESCALATION (fixture viewer cannot use admin routes) ─
    // Uses the fixture viewer to verify that non-admin users cannot call admin
    // write routes.  The fixture viewer's own ID is used as the target so no
    // real user row is passed as a path parameter.
    console.log("\n── Admin Privilege Escalation (fixture viewer cannot mutate admin routes) ──");
    const v2 = authed(viewerCookie);

    await check("POST /api/admin/users                   [viewer → 403]",
      v2("/api/admin/users", { method: "POST", body: JSON.stringify({ name: "x", email: "x@x.com" }) }), 403);
    await check(`POST /api/admin/users/${fixtureViewerId}/suspend    [viewer → 403]`,
      v2(`/api/admin/users/${fixtureViewerId}/suspend`, { method: "POST", body: JSON.stringify({}) }), 403);
    await check(`POST /api/admin/users/${fixtureViewerId}/reset-password [viewer → 403]`,
      v2(`/api/admin/users/${fixtureViewerId}/reset-password`, { method: "POST", body: JSON.stringify({ newPassword: "newpass1" }) }), 403);
    await check(`DELETE /api/admin/users/${fixtureViewerId}          [viewer → 403]`,
      v2(`/api/admin/users/${fixtureViewerId}`, { method: "DELETE" }), 403);
    await check(`PUT /api/admin/users/${fixtureViewerId}             [viewer → 403]`,
      v2(`/api/admin/users/${fixtureViewerId}`, { method: "PUT", body: JSON.stringify({ name: "x" }) }), 403);

  } finally {
    // Delete fixture viewer user unconditionally — crash-safe cleanup.
    if (fixtureViewerId !== null) {
      await pool.query(`DELETE FROM users WHERE id = $1`, [fixtureViewerId]).catch((e) => {
        console.warn(`teardown: could not delete fixture viewer (id=${fixtureViewerId}): ${e.message}`);
      });
      console.log(`\nTeardown: fixture viewer id=${fixtureViewerId} deleted.`);
    }
    await pool.end();
  }
