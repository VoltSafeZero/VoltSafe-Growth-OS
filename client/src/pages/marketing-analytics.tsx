...
import {
  ...
  AlertTriangle, ChevronDown, ChevronUp, Zap, DollarSign, Building2,
} from "lucide-react";
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-8">
        {isLoading ? (
          ...
          <>
            ...
            {/* ── Campaign ROI / Pipeline Attribution ──────────────────────────── */}
            <CampaignROISection />
...
function confidencePill(conf: string | null) {
  if (!conf) return null;
  const cls = conf === "high"
  ...
  return <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium ${cls}`}>{conf}</span>;
}
...
function CampaignROISection() {
  ...
  return (
    <div data-testid="campaign-roi-section">
      <div className="flex items-center gap-2 mb-4">
        <DollarSign className="w-4 h-4 text-cyan-400" />
        <h2 className="text-sm font-semibold text-foreground">Campaign ROI / Pipeline Attribution</h2>
        <Badge variant="outline" className="text-[10px] h-4 px-1.5 text-muted-foreground border-border/50 ml-1">
          Multi-touch
        </Badge>
      </div>
      ...
      {!isLoading && !isError && (
        <>
          {!hasAnyRevenue && (
            <div className="rounded-lg border border-dashed border-border/40 bg-muted/10 px-4 py-3 mb-4 text-xs text-muted-foreground" data-testid="no-revenue-data">
          ...
          {campaigns.length === 0 ? (
            <div className="rounded-xl border border-border/40 bg-muted/10 px-5 py-10 text-center mb-6" data-testid="attribution-campaigns-empty">
              ...
              <p className="text-xs text-muted-foreground/60 mt-1">
                Attribution events are recorded automatically from campaign reply classifications.
              </p>
            </div>
          ) : (
            <div className="mb-6">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Campaign Attribution</div>
              <div className="rounded-xl border border-border/50 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-xs min-w-[900px]">
                    <thead>
                      <tr className="border-b border-border/50 bg-muted/30">
                        {["Campaign", "Sent", "Replies", "Tasks", "Meetings", "Opps", "Proposals", "Pipeline", "Won Revenue", "Confidence"].map(h => (
                    ...
                    <tbody>
                      {campaigns.map((c, i) => (
                        <tr key={c.campaign_id}
                          ...
                          data-testid={`attribution-row-${c.campaign_id}`}
                        >
                          ...
                          <td className="px-3 py-2.5 font-mono text-muted-foreground">{(c.sent_count ?? 0).toLocaleString()}</td>
                          <td className="px-3 py-2.5 font-mono text-muted-foreground">{(c.replied_count ?? 0).toLocaleString()}</td>
                          <td className="px-3 py-2.5 font-mono">{c.tasks > 0 ? <span className="text-violet-400 font-medium">{c.tasks}</span> : <span className="text-muted-foreground/40">—</span>}</td>
                          <td className="px-3 py-2.5 font-mono">{c.meetings > 0 ? <span className="text-emerald-400 font-medium">{c.meetings}</span> : <span className="text-muted-foreground/40">—</span>}</td>
                          <td className="px-3 py-2.5 font-mono">{c.opportunities > 0 ? <span className="text-cyan-400 font-medium">{c.opportunities}</span> : <span className="text-muted-foreground/40">—</span>}</td>
                          <td className="px-3 py-2.5 font-mono">{c.proposals > 0 ? <span className="text-blue-400 font-medium">{c.proposals}</span> : <span className="text-muted-foreground/40">—</span>}</td>
                          <td className="px-3 py-2.5 font-mono">{fmt$(c.pipeline_value)}</td>
                          <td className="px-3 py-2.5 font-mono">{fmt$(c.won_revenue)}</td>
                          <td className="px-3 py-2.5">{confidencePill(c.top_confidence)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          {personas.length > 0 && (
            <div className="mb-6">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                <Building2 className="w-3 h-3" /> Persona Attribution
              </div>
              <div className="rounded-xl border border-border/50 overflow-hidden">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border/50 bg-muted/30">
                      {["Marina Persona", "Campaigns", "Engaged Accounts", "Opportunities", "Pipeline", "Won Revenue"].map(h => (
                        <th key={h} className="text-left px-3 py-2.5 text-[10px] font-medium text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {personas.map((p, i) => (
                      <tr key={p.persona}
                        className={`border-b border-border/20 ${i % 2 === 0 ? "" : "bg-muted/10"}`}
                        data-testid={`persona-row-${i}`}
                      >
                        <td className="px-3 py-2.5 font-medium text-foreground">{p.persona}</td>
                        <td className="px-3 py-2.5 font-mono text-muted-foreground">{p.campaigns}</td>
                        <td className="px-3 py-2.5 font-mono text-muted-foreground">{p.engaged_accounts}</td>
                        <td className="px-3 py-2.5 font-mono">{p.opportunities > 0 ? <span className="text-cyan-400">{p.opportunities}</span> : "—"}</td>
                        <td className="px-3 py-2.5 font-mono">{fmt$(p.pipeline_value)}</td>
                        <td className="px-3 py-2.5 font-mono">{fmt$(p.won_revenue)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {stakeholders.length > 0 && (
            <div>
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2 flex items-center gap-1.5">
                <Users className="w-3 h-3" /> Stakeholder Attribution
              </div>
              <div className="rounded-xl border border-border/50 overflow-hidden">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border/50 bg-muted/30">
                      {["Role", "Replies → Tasks", "Meetings", "Opportunities", "Influenced Pipeline"].map(h => (
                        <th key={h} className="text-left px-3 py-2.5 text-[10px] font-medium text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {stakeholders.map((s, i) => (
                      <tr key={s.role}
                        className={`border-b border-border/20 ${i % 2 === 0 ? "" : "bg-muted/10"}`}
                        data-testid={`stakeholder-row-${i}`}
                      >
                        <td className="px-3 py-2.5 font-medium text-foreground">{s.role}</td>
                        <td className="px-3 py-2.5 font-mono">{s.replies_with_tasks > 0 ? <span className="text-violet-400">{s.replies_with_tasks}</span> : "—"}</td>
                        <td className="px-3 py-2.5 font-mono">{s.meetings > 0 ? <span className="text-emerald-400">{s.meetings}</span> : "—"}</td>
                        <td className="px-3 py-2.5 font-mono">{s.opportunities > 0 ? <span className="text-cyan-400">{s.opportunities}</span> : "—"}</td>
                        <td className="px-3 py-2.5 font-mono">{fmt$(s.pipeline_value)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {!hasAnyEvents && campaigns.length > 0 && (
            <div className="mt-4 rounded-lg border border-dashed border-border/40 bg-muted/10 px-5 py-6 text-center text-xs text-muted-foreground" data-testid="no-attribution-events">
              No attribution events recorded yet. Events are created automatically when campaign replies generate tasks or meetings.
            </div>
          )}
        </>
      )}
